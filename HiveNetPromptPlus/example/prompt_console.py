...
from HiveNetPromptPlus import PromptPlusConsoleApp
...
def common_cmd_dealfun(message='', cmd='', cmd_para='', prompt_obj=None, **kwargs):
    ...
    if cmd == 'wait':
        _i = 0
        while _i < 10:
            # _logger.info('wait ' + str(_i))  # 该模型输出信息打破了界面位置
            print('wait ' + str(_i))  # 该模式会卡死界面，不是实时输出
            _i = _i + 1
            time.sleep(1)
...
if __name__ == '__main__':
    ...
    # 外部程序自己控制命令循环
    #############################
    prompt1 = PromptPlusConsoleApp(
        message='请输入>',
        default='help',
        enable_color_set=False,
        win_input_para={'height': 1, 'wrap_lines': True},
    ...
    # 自动命令循环模式-同步
    #############################
    prompt1.start_prompt_service(text=tips)
