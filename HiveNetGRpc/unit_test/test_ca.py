...
class TestGRpcJsonService(unittest.TestCase):
    ...
    def test_server_ssl_async(self):
        ...
        with AIOGRpcClient({
                'host': 'localhost', 'port': 50052, 'ping_on_connect': False, 'ping_with_health_check': True,
                'use_sync_client': False, 'timeout': 3
        }) as _client:
    ...
    def test_double_ssl_async(self):
        ...
        with AIOGRpcClient({
                'host': 'localhost', 'port': 50053, 'ping_on_connect': False, 'ping_with_health_check': True,
                'use_sync_client': False, 'timeout': 3
        }) as _client:
            TestFunction.test_server_ssl_failed(self, _client, True)
