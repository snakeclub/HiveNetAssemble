...
```
# 定义主进程使用的logger
_logger = HiveNetCore.logging.Logger(conf_file_name=None, logger_name=HiveNetCore.logging.EnumLoggerName.Console,
                 config_type=HiveNetCore.logging.EnumLoggerConfigType.JSON_STR)


# 定义并发任务的logger创建参数
_logger_kwargs = {
    'conf_file_name': None,
    'logger_name': HiveNetCore.logging.EnumLoggerName.Console,
    'config_type': HiveNetCore.logging.EnumLoggerConfigType.JSON_STR
}
...
2、支持指定队列发起并行任务（task_queue，队列对象应为Python原生队列对象，或基于HiveNetCore.queue.QueueFw扩展的队列对象）；如果需要执行固定数量的任务，可以将任务放入队列后，指定自动结束参数（auto_stop），再启动并发池，并发池将在任务完成后自行结束；
