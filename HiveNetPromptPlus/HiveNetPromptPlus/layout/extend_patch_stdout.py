...
@module extend_patch_stdout
@file extend_patch_stdout.py
"""
...
@contextmanager
def extend_patch_stdout(sleep_between_writes: float = 0.2,
        raw: bool = False,
        output: Output = None) -> Generator[None, None, None]:
    """
    ...
    :param raw: (`bool`) When True, vt100 terminal escape sequences are not
                removed/escaped.
    :param output: 自定义的output处理对象, 继承 DummyOutput 实现即可
    """
    with ExtendStdoutProxy(
        sleep_between_writes=sleep_between_writes, raw=raw, output=output
    ) as proxy:
...
class ExtendStdoutProxy:
    ...
    def __enter__(self) -> "ExtendStdoutProxy":
        return self
    ...
    def _write_and_flush(
        ...
        def write_and_flush_in_loop() -> None:
            ...
            # call it directly.
            run_in_terminal(write_and_flush, in_executor=False)  # 会导致闪屏
